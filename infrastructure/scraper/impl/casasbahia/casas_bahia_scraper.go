...
const (
	storeName          = "casasbahia"
	casasBahiaBaseURL = "https://www.casasbahia.com.br"
)

var pricePattern = regexp.MustCompile(`R\$\s*([0-9.]+,[0-9]{2})`)

func NewCasasBahiaScraper(url string) scraper.Scraper {
...
func (s *CasasBahiaScraper) Name() string {
	return storeName
}

func (s *CasasBahiaScraper) Search(ctx context.Context, query string) ([]entity.Product, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("casasbahia.Search: criar request: %w", err)
	}

	req.Header.Set("User-Agent", "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36")
	req.Header.Set("Accept-Language", "pt-BR,pt;q=0.9")
	req.Header.Set("Referer", casasBahiaBaseURL+"/")

	resp, err := s.client.Do(req)
	if err != nil {
		return nil, fmt.Errorf("casasbahia.Search: executar request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("casasbahia.Search: status inesperado: %d", resp.StatusCode)
	}

	doc, err := goquery.NewDocumentFromReader(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("casasbahia.Search: parsear HTML: %w", err)
	}

	return s.normalize(doc), nil
}

func (s *CasasBahiaScraper) normalize(doc *goquery.Document) []entity.Product {
...
func isProductCard(selection *goquery.Selection) bool {
	...
	testID := strings.ToLower(attr(selection, "data-testid"))
	class := strings.ToLower(attr(selection, "class"))

	return strings.Contains(testID, "product") ||
		strings.Contains(testID, "card") ||
...
func absoluteURL(rawURL string) string {
	...
	if strings.HasPrefix(rawURL, "/") {
		return casasBahiaBaseURL + rawURL
	}
	return rawURL
}
...
func canonicalProductURL(rawURL string) string {
	...
	if parsed.Host != "www.casasbahia.com.br" && parsed.Host != "casasbahia.com.br" {
		return ""
	}
	if !isProductPath(parsed.Path) {
		return ""
	}
	...
	parsed.Fragment = ""
	parsed.Scheme = "https"
	parsed.Host = "www.casasbahia.com.br"
...
func isProductPath(path string) bool {
	...
	return strings.Contains(path, "/p/") ||
		strings.HasPrefix(path, "produto/") ||
